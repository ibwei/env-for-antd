...
class MFilterClass extends Vue {
  ...
  gradeSwitch(val: boolean): void {
    this.showGrade = val
    this.tableMarginTop = val ? (this.$refs.filterGrade as Element).clientHeight : (this.$refs.filterNormal as Element).clientHeight
  ...
  formItem(getFieldDecorator: any, item: FilterFormList, index: number, grade?: boolean) {
    ...
    switch (item.type) {
      ...
      case 'select':
        ...
        break
      case 'cascader':
        itemDom = <a-cascader style="width: 100%;" id={item.key} allowClear changeOnSelect fieldNames={item.fieldNames} options={item.options} placeholder={item.placeholder} on-change={item.change}></a-cascader>
        break
      case 'levelcode':
        itemDom = <a-cascader style="width: 100%;" id={item.key} allowClear changeOnSelect fieldNames={item.fieldNames} options={item.options} placeholder={item.placeholder} on-change={(e: Array<string>) => this.levelcodeChange(e, item.key)}></a-cascader>
        break
      case 'datetime':
        itemDom = <a-date-picker id={item.key} showTime format="YYYY-MM-DD HH:mm:ss" placeholder={item.placeholder}></a-date-picker>
        break
      case 'date':
        itemDom = <a-date-picker id={item.key} format="YYYY-MM-DD" placeholder={item.placeholder}></a-date-picker>
        break
      case 'datetimerange':
        itemDom = <a-range-picker style="width: 100%" id={item.key} showTime format="YYYY-MM-DD HH:mm:ss" disabledTime={item.disabledTime} on-change={(e: Array<Date>) => this.rangeChange(e, item.value ? item.value : [])} placeholder={item.placeholder}></a-range-picker>
        break
      case 'checkboxButton':
        itemDom = (
